...
def id_lambda(sess, model, model2, original_img, target1):
    ...
    for i in range(1000):
        ...
        # if sess.run(loss1)<0.2 and sess.run(loss2)>=150:
        #      print('GG')
        #      break
    # np.save("saved_point/img.npy")

    return sess.run(loss_1/loss_2)
